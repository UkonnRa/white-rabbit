...
impl SeaOrmJournalRepository {
    ...
    async fn load_tags_batch(
        ...
        let tags = sess
            .query_all(tag_entity::Entity::find().filter(tag_entity::Column::JournalId.is_in(ids)))
            .await?;
    ...
    async fn save_tags_batch(
        ...
        let ids: Vec<String> = journals.iter().map(|j| j.id.value().to_string()).collect();

        sess.exec_delete(
            tag_entity::Entity::delete_many().filter(tag_entity::Column::JournalId.is_in(ids)),
        )
        .await?;
    ...
    async fn delete_tags_batch(
        ...
        let values: Vec<String> = ids.iter().map(|id| id.value().to_string()).collect();
        sess.exec_delete(
            tag_entity::Entity::delete_many().filter(tag_entity::Column::JournalId.is_in(values)),
        )
        .await?;
        Ok(())
    }
