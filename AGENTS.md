...
- Do not introduce language- or framework-specific assumptions into architecture docs.
...
- Use DDD boundaries: domain rules live in aggregates/value objects, not in infrastructure layers.
...
- Internal in-process domain reactions may run in dry-run when they are pure/in-transaction effects.
- Command-time decision queries must read from the write-consistent model, not eventually-consistent read models.
- CQRS is allowed with different read/write executors, but shared query semantics should be expressed via Specification.
...
- Keep project/domain errors contextual and rich (entity type, field, offending value, etc.).
- Map generic errors to contextual domain/application errors at boundaries.
- API-facing error payloads should be machine-readable and standards-aligned (Problem Details / JSON:API style).
...
- New architectural constraints require: (1) ADR, (2) architecture doc update, (3) this file update if enforceable.
...
- For CQRS-related changes, verify write-side correctness before read-model performance concerns.
