...
- **New tests** need no extra config — the `build.rs` fix applies to all test
  targets automatically.
- **Removal:** once upstream fixes land, delete
  `tests/comctl32-v6.exe.manifest`, strip the `#[cfg(windows)]` block from
  `build.rs`, and verify on Windows.
